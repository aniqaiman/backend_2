    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">Add Vegetable</h4>
            </div>

            <form action="{{ action('VegetableController@store') }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
                @csrf
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary">Add New Vegetable</button>
                </div>
            </form>
        </div>
    </div>
...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Manage Vegetables</li>
    </ol>
</section>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="text-center">
                                        <img style="max-height: 100px;" src="{{ env('APP_PHOTO_URL') }}{{ $vegetable->image }}" onerror="this.src='/img/no-image.jpg'" />
                                    </td>
                                    <td>{{ $vegetable->name }}</td>
                                    <td>
                                        <dl>
                                            <dt>Short</dt>
                                            <dd>{{ isset($vegetable->short_description) ? $vegetable->short_description : '-' }}</dd>
                                            <dt>Full</dt>
                                            <dd>{{ isset($vegetable->description) ? $vegetable->description : '-' }}</dd>
                                        </dl>
                                    </td>
                                    <td class="text-center text-nowrap">{{ $vegetable->quantity_a }} kg</td>
                                    <td class="text-center text-nowrap">{{ $vegetable->quantity_b }} kg</td>
                                    <td class="text-center text-nowrap">{{ $vegetable->demand_a }} kg</td>
                                    <td class="text-center text-nowrap">{{ $vegetable->demand_b }} kg</td>
                                    <td class="text-center text-nowrap">
                                        <form method="POST" action="{{ action('VegetableController@destroy', $vegetable->id) }}" class="btn-group-vertical btn-group-sm">
                                            @csrf
                                            <input type="hidden" name="_method" value="DELETE" />
                                            <div class="btn-group-vertical btn-group-sm">
                                                <a class="btn btn-info" href="{{ route('products.vegetables.edit', ['product_id'=> $vegetable->id]) }}">Edit</a>
